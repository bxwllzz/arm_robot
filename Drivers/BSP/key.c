...
static struct KeyStateStatus Statusstate = { NoKeyDown , NoKeyDown ,  ContinueNoKeyDown ,  ContinueNoKeyDown };
static struct Continue_Key_Down_StateStatus Continue_Key_Down_state = { Continue_No_Key_Down, Continue_No_Key_Down };
static struct Continue_Key_Release_StateStatus Continue_Key_Release_state = { Continue_No_Key_Release, Continue_No_Key_Release };
...
static Continue_State_Release_Status Buttom_Status_Continue_Release(GPIO_TypeDef* GPIOx,uint16_t GPIO_Pin,uint8_t Down_state)
{
	...
	uint32_t KeyPress  = 1 ;
	Continue_State_Release_Status state_Continue_Release = Continue_No_Key_Release;
	if( HAL_GPIO_ReadPin(GPIOx,GPIO_Pin) != Down_state )   //读取按键输入
	{
		KeyPress = 0;
		if( GPIOx == GPIOA && GPIO_Pin == GPIO_PIN_1)
...
{
	if( Buttom_Status_Continue_Down( GPIOA, GPIO_PIN_0, Down_State_key ) ==  Continue_Key_Down )
	{
	}
	if( Buttom_Status_Continue_Release( GPIOA, GPIO_PIN_0, Down_State_key ) ==  Continue_Key_Down )
	{
	}
}
...
{
	if( Buttom_Status_Continue_Down( GPIOA, GPIO_PIN_1, Down_State_key ) ==  Continue_Key_Down )
	{
	}
	if( Buttom_Status_Continue_Release( GPIOA, GPIO_PIN_1, Down_State_key ) ==  Continue_Key_Down )
	{
	}
}
